...
engine = create_engine('postgresql://gzpdseeqgilgmc:f8612aed7a618933700c870247c15f70961b0a366796095091c46b8be75790b8'
                       '@ec2-107-21-99-237.compute-1.amazonaws.com:5432/dasmon190o3648', echo=True)
...
def send_otp():
    if request.method == 'POST':
        ...
        # get response
        response = sendPostRequest(URL, 'CX97133NP0QNNM9MSNSE0QR7A05GS72A', 'MDV7RXBWBZL593EX', 'stage', mob_no, '1',
                                   otp)
    ...
    if request.method == 'POST':
        ...
        if user_field == 'student':
            ...
            if db.execute("SELECT number FROM student WHERE number = :stu_no", {"stu_no": number}).rowcount == 0:
                db.execute(
                    "INSERT INTO student (name,sex,dob,email,number,aadhar,pincode,qualification,game,password) VALUES (:s1,:s2,:s3,:s4,:s5,:s6,:s7,:s8,:s9,:s10)",
                    {"s1": name, "s2": sex, "s3": dob, "s4": email, "s5": number, "s6": aadhar, "s7": pincode,
                     "s8": qualification, "s9": game, "s10": password})
                db.commit()
            ...
            if db.execute("SELECT number FROM player WHERE number = :ply_no", {"ply_no": number}).rowcount == 0:
                db.execute(
                    "INSERT INTO player (name,sex,dob,email,number,aadhar,pincode,game,level,achievement,job,password) VALUES (:s1,:s2,:s3,:s4,:s5,:s6,:s7,:s8,:s9,:s10,:s11,:s12)",
                    {"s1": name, "s2": sex, "s3": dob, "s4": email, "s5": number, "s6": aadhar, "s7": pincode,
                     "s8": game, "s9": level, "s10": achivement, "s11": job, "s12": password})
                db.commit()
            ...
            if db.execute("SELECT number FROM student WHERE number = :app_no", {"app_no": number}).rowcount == 0:
                db.execute(
                    "INSERT INTO applicant (name,sex,dob,email,number,aadhar,pincode,qualification,area,exprience,achievement,password) VALUES (:s1,:s2,:s3,:s4,:s5,:s6,:s7,:s8,:s9,:s10,:s11,:s12)",
                    {"s1": name, "s2": sex, "s3": dob, "s4": email, "s5": number, "s6": aadhar, "s7": pincode,
                     "s8": qualification, "s9": area, "s10": exprience, "s11": achievement, "s12": password})
                db.commit()
                return render_template('login-system/signin.html', msg='Account create successfull, Signin now!')
            else:
                return render_template('login-system/signin.html', msg="Your account exist, Signin now!")
            ...
            if db.execute("SELECT number FROM goverment WHERE number = :gov_no", {"gov_no": number}).rowcount == 0:
                db.execute(
                    "INSERT INTO goverment (name,email,number,aadhar,pincode,password) VALUES (:s1,:s2,:s3,:s4,:s5,:s6)",
    ...
    if request.method == 'POST':
        ...
        if field == 'student':
            ...
            if db.execute("SELECT number FROM applicant WHERE number = :number", {"number": number}).rowcount == 0:
                ...
                password_db = raw_password[0]
                if (password == password_db):
                    applicant = db.execute("SELECT * FROM applicant WHERE number = :stu_no",
                                           {"stu_no": number}).fetchone()
                    msg = db.execute("SELECT msg FROM gov_message WHERE receiver = :name",
                                     {"name": 'applicant'}).fetchall()
                    count = db.execute("SELECT count(msg) FROM gov_message WHERE receiver = :name",
                                       {"name": 'applicant'}).fetchall()
                    return render_template('dashboards/dash_applicant.html', applicant=applicant, msg=msg,
                                           count=count[0])
                else:
                    return render_template('login-system/signin.html', msg="Your enter incorrect password, Try again!")
...
def gov_action(id):
    if request.method == 'POST':
        ...
        if (act == 'accept'):
            ...
            URL = 'http://www.way2sms.com/api/v1/sendCampaign'
            response = sendPostRequest(URL, 'CX97133NP0QNNM9MSNSE0QR7A05GS72A', 'MDV7RXBWBZL593EX', 'stage', no, '1',
                                       'your application is accepted we send interview date soon')
